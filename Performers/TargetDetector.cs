...
// Класс собирает все хитбоксы в пределах триггера и позволяет запускать выборки среди этих объектов
public class TargetDetector : MonoBehaviour
{
        ...
        for (int i = 0; i < Objects.Count; i++)
            if (!exceptions.Contains(Objects[i]))
                if (Objects[i].Side == side)
                    if (Objects[i].PerformerType == type || type == PerformerType.Нечто)
    ...
    public List<Vulnerable> SelectAll(Side? side = null, PerformerType type = PerformerType.Нечто, float range = Mathf.Infinity, Vector3 targetPosition4AngleCount = new Vector3(), Vulnerable exception = null)
    {
        FixedUpdate();
